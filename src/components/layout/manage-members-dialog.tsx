        ...
        <div className="bg-white rounded-hand-xl shadow-md w-full max-w-sm animate-slide-up" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="max-h-64 overflow-y-auto px-4 py-2 space-y-1">
            {members.map((member) => (
              <div
                key={member.id}
                className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-mint-50 transition-colors"
              >
                {editingId === member.id ? (
                  <>
                    <input
                      ...
                      className="flex-1 text-sm px-2 py-1 border border-surface-300 rounded focus:outline-none focus:ring-2 focus:ring-mint-200"
                      autoFocus
                    />
                    <button
                      onClick={() => handleSaveEdit(member.id)}
                      className="p-1 text-mint-600 hover:bg-mint-50 rounded transition-colors"
                    >
                  ...
                  <>
                    <div className="w-6 h-6 rounded-full bg-mint-100 text-navy-700 flex items-center justify-center text-[10px] font-medium">
          ...
          {/* Add member */}
          <div className="flex items-center gap-2 px-4 py-3 border-t border-surface-200/50">
            <input
              type="text"
